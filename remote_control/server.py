...
# - 현재 버전: 1.21.13
# - 수정 기록:
#   1.21.13: (이 파일 자체는 변경 없음, 버전 동기화용) 단일 실행 가드 + 빈사 정체 탈출 가드 완치 +
#     힐링 후 필드 판정 교정. 상세는 history.log 참고.
#   1.21.12: (이 파일 자체는 변경 없음, 버전 동기화용) 정체 증거 스크린샷 자동 저장 + 미니게임 왕복
#     전용 정체 카운터(10회) + "대화창 저격" 선택지 우선 처리. 상세는 dungeon_bot.py/main.py 참고.
#   1.21.11: (이 파일 자체는 변경 없음, 버전 동기화용) 눈보라구간/정체탈출가드 후속 완치 3건(재개
#     버튼 상대밝기 판정, 정체 탈출 가드 1회 힐 제한 등) + 던전 진입 대기 15초 상향 + 부팅 복구
#     분류기 전투 인식 임계값 하향. 상세는 dungeon_bot.py/main.py 참고.
#   1.21.10: (이 파일 자체는 변경 없음, 버전 동기화용) 전투 진입 직후 자동전투가 영원히 안 켜져
#     정체하던 결함 + 대설지대 눈보라구간 재개버튼 무한 반복 결함, 2건 완치. 상세는 dungeon_bot.py 참고.
#   1.21.9: (이 파일 자체는 변경 없음, 버전 동기화용) 빈사/사망 구분 실패로 인한 정비 무한 재격발
#     완치 - 사망 감지 도장/좌표 재검증까지 완료(정탐 0.999~1.000 vs 오탐 최고 0.585). 상세는
#     dungeon_bot.py 참고.
#   1.21.8: (이 파일 자체는 변경 없음, 버전 동기화용) 예상 밖 상태 팝업으로 인한 정체를 능동적으로
#     해소하는 안전장치 추가. 상세는 dungeon_bot.py 참고.
#   1.21.7: (이 파일 자체는 변경 없음, 버전 동기화용) 대설지대 "뼈상인" 조우 처리 신규 추가 -
#     모험가의 뼈 1순위, 유해를 부르는 기름 2순위 고정 선택. 상세는 dungeon_bot.py 참고.
#   1.21.6: (이 파일 자체는 변경 없음, 버전 동기화용) 대설지대 주회 한도 도달 시 무한 왕복 완치 -
#     한도 도달 시 실제로 여관을 경유하도록 근본 완치. 상세는 main.py 참고.
#   1.21.5: (이 파일 자체는 변경 없음, 버전 동기화용) 대설지대 상자파밍 중 터치가 완전히 죽어도
#     300초 워치독이 발동 못 해 3시간 넘게 재시작이 안 걸리던 결함 완치. 상세는 dungeon_bot.py 참고.
#   1.21.4: (이 파일 자체는 변경 없음, 버전 동기화용) recover_app_startup()이 대설지대 "마을외곽"
#     화면을 프리셋에 따라 반쪽만 인식하던 결함 완전 완치. 상세는 main.py 참고.
#   1.21.3: (이 파일 자체는 변경 없음, 버전 동기화용) 캠프 아이콘이 화면 상단 타이틀바에 가까울 때
#     자동이동 버블이 가려지던 결함 + 그 실패가 곧장 앱 재시작으로 이어지던 과잉 대응 완화. 상세는
#     dungeon_bot.py 참고.
#   1.21.2: (이 파일 자체는 변경 없음, 버전 동기화용) 필드맵 귀환 루틴의 캠핑 화면 미감지 결함
#     완치 + 자동이동 대기 루프의 "우물말랐다" 화살표 가로채기(도착 판정 무한 반복) 완치 + 하켄
#     분기의 아이콘 미검출 나가기 폴백 비대칭 완치. 상세는 dungeon_bot.py 참고.
#   1.21.1: (이 파일 자체는 변경 없음, 버전 동기화용) 정체 타이머 리셋 누락 2건 완치. 상세는
#     dungeon_bot.py 참고.
#   1.21.0: (이 파일 자체는 변경 없음, 버전 동기화용) ADB 화면 캡처를 원시(raw) 방식으로 전환해
#     캡처 속도를 약 2.5배 개선(1.60초→0.63초). 상세는 main.py/dungeon_bot.py/screen_capture.py 참고.
#   1.20.0: (이 파일 자체는 변경 없음, 버전 동기화용) 뮤뮤 안드로이드 15 전용화 + 뮤뮤 '앱 상주'
#     설정으로 인한 화면 캡처/클릭 오작동 완치(연결 직후 진단 후 정지) + 대설지대 6층 실전 완주
#     검증 과정에서 발견된 세계지도/전투/캠핑 관련 결함 다수 완치. 상세는 main.py/dungeon_bot.py 참고.
#   1.19.2: (이 파일 자체는 변경 없음, 버전 동기화용) 대설지대 던전 추가 전 마지막 안정화 릴리즈 - ADB
#     연결 로그 정리, 콘솔창 잔존/스크린샷 캐시 무한증식/정체카운터 리셋 누락 완치. 상세는
#     main.py/dungeon_bot.py 참고.
#   1.19.1: (이 파일 자체는 변경 없음, 버전 동기화용) 재부팅 로그 파일명 reboot2+ 미표기 결함 완치 및
#     유령성4층 착지 스턱 즉시 스와이프/딜레이 단축. 상세는 main.py/dungeon_bot.py 참고.
