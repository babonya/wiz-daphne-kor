...
> **WizDapKor_v1.20.0 안내** (뮤뮤 안드로이드 15 전용화 + 화면 인식 오작동 완치)
>
> ## ⚠️ 지원 환경 변경 공지
> **이 버전부터 MuMu Player 안드로이드 15 인스턴스만 지원합니다.** 안드로이드 12 인스턴스에서는 동작하지
> 않으며, 매크로가 안드15 인스턴스를 찾지 못하면 어느 포트가 안드로이드 몇인지 알려준 뒤 안내를 띄우고
...
> 달라집니다** — 게임 대신 안드로이드 홈 화면을 마구 누르게 됩니다. 이제 매크로가 시작할 때 이 상태를
...
>
> - **안드15에서 매크로가 아무것도 못 하던 결함 완치**: 위 설정 때문에 화면을 찍어올 때 그림 데이터 앞에
>   안내 문구가 섞여 들어와, 매크로가 화면을 아예 못 읽고 재시작만 반복하던 문제입니다. 원인과 해결법을
>   시작할 때 바로 알려주도록 했고, 혹시 문구가 섞여도 그림은 읽히도록 안전장치도 넣었습니다.
