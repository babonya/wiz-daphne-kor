...
> **WizDapKor_v1.21.5 안내** (대설지대 던전 신규 지원 + 실전 안정화, 캡처 속도 2.5배 향상)
>
...
>
> - **장시간 방치 중 터치 먹통으로 인한 무한 정체 완치**: 드물게 에뮬레이터의 터치 입력만 멎고 화면
>   캡처는 정상인 상황에서, 이동 감지 로직이 이를 "정상 이동 중"으로 오판해 정체 감지/자동 재시작이
>   걸리지 않던 결함을 완치했습니다(실전에서 3시간 넘게 같은 자리에 머물러도 재시작이 안 걸리던
>   사례로 확인).
> - **다른 위치/프리셋으로 켜도 목표 던전을 잘 찾아가도록 보강**: 대설지대 마을외곽 화면에 캐릭터가
>   있는데 다른 던전 프리셋으로 매크로를 켜면, 기동 복구가 그 화면을 인식하지 못해 멈춰있던 결함을
>   완치했습니다.
> - **대설지대(Heavysnow) 던전 신규 지원 + 실전 안정화**: 새 던전(대설지대) 6층/교회구역 자동 파밍을
>   지원합니다. 미니맵을 열어 캠프(우물)/대하켄 아이콘을 찾아가는 새로운 방식의 귀환 루틴을 실전에서
>   반복 검증하며 여러 결함을 완치했습니다 - 캠핑 화면을 못 알아채고 제자리에서 헛돌던 문제, 캠핑이
>   이미 끝난 자리에서 재진입했을 때 대화창이 무한 반복되던 문제, 하켄이 없는 구역에서 탈출 수단이
>   아예 없던 문제, 우물 아이콘이 화면 상단에 가까울 때 자동이동 버튼을 못 찾던 문제 등입니다.
> - **화면 캡처 속도 약 2.5배 향상**: 화면을 읽어오는 방식을 압축(PNG) 대신 무압축(원시) 방식으로
>   바꿔, 평균 1.6초 걸리던 캡처가 0.6초로 줄었습니다. 매크로 전체의 반응 속도가 눈에 띄게 빨라집니다.
> - **정체 오판 완치**: 힐링/상자 처리 직후 실제로는 화면이 잘 진행됐는데도 "정체"로 잘못 판단해
>   불필요한 복구 동작이 나가던 결함 2건을 완치했습니다.
> - **회복 가능한 상황에서의 과잉 재시작 완화**: 연결/화면인식 자체는 멀쩡한 일시적 상황에서도 곧장
>   전체 재시작으로 처리하던 지점들을 손봐, 가벼운 재시도로 먼저 회복을 시도하도록 완화했습니다.
> - **안드15에서 매크로가 아무것도 못 하던 결함 완치**: '앱 상주' 설정 때문에 화면을 찍어올 때 그림
>   데이터 앞에 안내 문구가 섞여 들어와, 매크로가 화면을 아예 못 읽고 재시작만 반복하던 문제입니다.
>   원인과 해결법을 시작할 때 바로 알려주도록 했고, 혹시 문구가 섞여도 그림은 읽히도록 안전장치도
>   넣었습니다.
>
> <details>
> <summary>이전 버전(1.20.0) 변경 이력 보기</summary>
>
> **v1.20.0** (뮤뮤 안드로이드 15 전용화 + 화면 인식 오작동 완치)
> - **안드15에서 매크로가 아무것도 못 하던 결함 완치**: 위 '앱 상주' 설정 때문에 화면을 찍어올 때 그림
>   데이터 앞에 안내 문구가 섞여 들어와, 매크로가 화면을 아예 못 읽고 재시작만 반복하던 문제입니다.
> - **접속 대상 자동 선별**: 안드로이드 12 인스턴스가 같이 켜져 있어도 그쪽에 잘못 붙지 않고, 안드로이드
>   15 인스턴스를 찾아서 접속합니다.
>
> </details>
>
