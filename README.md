...
> **WizDapKor_v1.21.13 안내** (대설지대 던전 신규 지원 + 화면 인식 속도 개선)
>
> 대설지대를 처음 담은 공개 릴리즈입니다. 큰 변경사항만 정리합니다.
>
> ## 🚨 v1.21.13 긴급 수정 (핫픽스)
> 네 가지를 고쳤습니다.
> - **빈사 정체 무한 반복 완치**: 빈사(딸피) 상태로 화면 인식이 흔들리면 "힐링 1회 → 그래도 안 풀리면
>   나가기"로 넘어가야 하는데, 카운터가 매번 초기화돼 힐링 재시도만 몇 시간씩 반복하다 뒤로가기가 쌓여
>   마을까지 밀려나는 경우가 있었습니다. 이제 정상적으로 나가기 단계로 넘어갑니다.
> - **매크로 중복 실행 방지**: 어떤 경로로든(배치파일을 두 번 누르거나 원격 시작이 겹치는 등) 매크로가
>   두 개 이상 동시에 뜨지 않도록, 나중에 뜬 쪽이 스스로 종료합니다.
> - **힐링 후 오터치 완치**: 미니맵을 접어둔 필드에서 힐링을 마친 뒤, 필드를 못 알아보고 엉뚱한 좌표를
>   누를 수 있던 문제를 고쳤습니다.
> - **새 글꼴의 '재시도'/'타이틀로' 버튼 인식**: 게임 업데이트로 글꼴이 바뀐 오류 팝업 버튼도 인식합니다.
> - 기본 설정 변경: 전투가 끝날 때마다 힐링(`HEALING_LOOPS = 1`).
>
> ## 🚨 v1.21.12 긴급 수정 (핫픽스)
> 세 가지를 추가로 고쳤습니다.
> - **정체 증거 스크린샷 자동 저장**: 매크로가 멈춘 순간의 화면을 자동으로 저장해, 다음에 비슷한
>   문제가 생기면 원인 파악이 훨씬 쉬워집니다.
> - **상자 미니게임 무한 반복 완치**: 뮤뮤 자체가 멈추는 등의 상황에서 상자 미니게임 화면을 두고
>   영원히 왕복하며 방치되던 경우가 있었습니다. 이제 일정 횟수(10회)를 넘으면 진짜 정체로 판단해
>   자동으로 재시작합니다.
> - **"대화창 저격" 오작동 완치**: 매크로 재부팅 직후 특정 선택지 화면(예: 울타리 구멍 조우)을 만나면
>   실제 선택 없이 화살표만 무의미하게 계속 누르며 몇 시간씩 방치되는 경우가 있었습니다. 이제 화살표를
>   누르기 전에 먼저 실제 선택지가 있는지 확인하고, 있으면 올바른 선택지를 고릅니다.
>
> ## 🚨 v1.21.11 긴급 수정 (핫픽스)
> 네 가지 정체 버그를 추가로 고쳤습니다.
> - **던전 진입 대기 시간 상향**: 대설지대 6층 등 일부 던전은 로딩이 10초를 살짝 넘는 경우가 있어,
>   던전 진입 대기 시간을 10초 → 15초로 늘렸습니다.
> - **눈보라구간 재개 버튼 오판정 완치**: 재개 버튼이 비활성 상태인데도 활성으로 잘못 판단해 계속
>   무의미하게 눌리는 경우가 있었습니다. 안개 농도에 따라 화면 밝기 자체가 흔들리는 문제였는데,
>   같은 화면의 "나가기" 버튼 밝기를 기준으로 상대 비교하도록 고쳐 안개 농도와 무관하게 정확히
>   판정하도록 개선했습니다.
> - **정체 탈출 가드 개선**: 정체 상태에서 "빈사" 판정 시 힐을 최대 3회까지 반복 시도하던 것을 1회로
>   줄이고, 그래도 안 풀리면 진짜 빈사가 아닌 것으로 보고 곧장 나가기를 시도하도록 바꿨습니다(모든
>   던전 공통 적용).
> - **부팅 복구 시 전투 화면 오판정 완치**: 동결(freeze) 상태이상이 걸린 전투 화면을 매크로 재부팅
>   직후 인식하지 못해 1시간 넘게 아무 것도 안 하고 방치되는 경우가 있었습니다. 인식 기준을 실전
>   화면으로 재검증해 완치했습니다.
>
> ## 🚨 v1.21.10 긴급 수정 (핫픽스)
> 두 가지 정체 버그를 고쳤습니다.
> - **전투 진입 후 자동전투 미점검 정체**: 전투가 시작되고 배속은 정상 전환되는데, 자동전투 토글
>   버튼의 모양이 최신 UI(전투패턴 번호 배지 추가)와 안 맞아 좌표를 못 찾아 자동전투가 계속 꺼진
>   채 방치되는 경우가 있었습니다. 이제 버튼 위치를 못 찾아도 검증된 고정 좌표로 바로 눌러 켭니다.
> - **대설지대 눈보라구간 재개버튼 무한 반복**: 눈보라 서브구역에서 재개 버튼이 반응 없이 계속 눌리기만
>   하며 무한정 갇히는 경우가 있었습니다. 이제 30초 넘게 안 풀리면 자동으로 나가기를 시도하고, 안전한
>   지점에서 캐릭터를 멈춘 뒤 정상 절차로 복귀합니다.
>
> ## 🚨 v1.21.9 긴급 수정 (핫픽스)
> 파티원이 완전히 사망했는데도 매크로가 이를 "빈사"(HP 낮음, 회복 가능)로 오인해 힐을 무한히
> 재시도하는 버그가 있었습니다. 사망은 힐로 되돌릴 수 없어 이 오판이 계속 반복되며 실제로 **12시간
> 동안 매크로가 헛돌기만 하는 사고**로 이어질 수 있었습니다. 이제 사망을 정확히 구분해, 사망자가
> 확인되면 힐 재시도 대신 마을 회군을 시도합니다(마을 복귀 시 확률적으로 부활하는 경우가 있어
> 시도합니다).
>
> ## 🚨 v1.21.6 긴급 수정 (핫픽스)
> 이전 버전(v1.21.5)에서 주회 횟수(`LIMIT_DUNGEON_LOOPS`)를 **0이 아닌 값**으로 설정하면, 정해진
> 주회를 다 돈 뒤 던전선택 화면에서 무한 루프에 빠지는 버그가 있어 긴급 수정했습니다. 기본값(0,
> 무제한 주회)으로 쓰셨다면 영향이 없었을 것이나, 주회 횟수를 직접 지정해 쓰셨다면 이 버전으로
> 업데이트해주세요.
>
> ## ⚠️ 지원 환경 변경 공지 (v1.20.0부터 적용)
> **MuMu Player 안드로이드 15 인스턴스만 지원합니다.** 안드로이드 12 인스턴스에서는 동작하지 않으며,
> 매크로가 안드15 인스턴스를 찾지 못하면 어느 포트가 안드로이드 몇인지 알려준 뒤 안내를 띄우고
> 중단합니다. (안드12 인스턴스는 그래픽 렌더러 에러가 반복돼 사실상 사용이 불가능해졌습니다.)
>
> ## 🚨 뮤뮤 필수 설정: '앱 상주'를 꺼주세요
> 뮤뮤 **[디바이스 설정] → [기타] → 맨 위 "애플리케이션이 실행 중입니다"** 항목
> (설명이 "여러 애플리케이션이 유휴 상태일 때 동시에 백그라운드에서 실행"인 것)을 **꺼야** 합니다.
> 이게 켜져 있으면 뮤뮤가 앱마다 별도의 화면을 만들어서, 매크로가 **읽는 화면과 클릭하는 화면이 서로
> 달라집니다** — 게임 대신 안드로이드 홈 화면을 마구 누르게 됩니다. 매크로가 시작할 때 이 상태를
> 감지하면 **끄는 방법을 알려주고 즉시 멈춥니다.**
>
> 아래처럼 **"꺼짐"** 상태여야 합니다:
>
> ![뮤뮤 앱 상주(백그라운드 실행) 끄기](readme_img/백그라운드실행_off.png)
>
> - **화면 인식 속도 대폭 향상**: 화면을 읽어오는 방식을 개선해 매크로 전체의 반응 속도가 눈에 띄게
>   빨라졌습니다(캡처 한 번에 평균 1.6초 → 0.6초).
> - **대설지대(Heavysnow) 던전 신규 지원**: 6층 자동 파밍을 지원합니다. 진행 순서는 **상자파밍 →
>   (상자가 떨어지면) 캠핑 → 마을외곽으로 귀환 → 여관은 들르지 않고 아이템만 보충 → 재진입**을
>   계속 반복하는 방식입니다.
> - **던전 안 대화 이벤트 자동 처리**: 이동 중 "수상한 행상인"을 만나면 망치를 구매하고, 중립몹과
>   마주치면 "싸운다"를 선택해 자동으로 넘깁니다. "뼈상인"(뼈 줍는 고블린)을 만나면 **모험가의 뼈를
>   1순위, 유해를 부르는 기름을 2순위**로 구매합니다.
> - **예상 밖 상태 팝업으로 인한 정체 완화**: 상자가 즉시 열리고 곧장 다음 전투로 이어지는 등 드문
>   상황에서 뜬 캐릭터 상태 팝업이 화면 인식을 가려 잠깐 멈추는 현상을, 팝업의 닫기 버튼을 능동적으로
>   눌러 해소하도록 개선했습니다.
>
> ## 🔭 다음 버전 예고
> ~~여전히 자동 스킬 구현은 미루고~~ GUI 기반 글로벌 설정 구역 & 프리셋 배치 편집기를 만들어볼까
> 고민 중입니다. 글로벌 설정이 점점 복잡해지고 있어서, `main.py`를 직접 안 열어도 고칠 수 있는
> 설정 화면이 슬슬 필요해진 상황입니다.
>
> <details>
> <summary>이전 버전(1.19.2) 변경 이력 보기</summary>
>
> **v1.19.2** (대설지대 던전 추가 전 마지막 안정화 릴리즈)
> - **ADB 연결 로그 정리**: 매크로 시작 시 `adb connect`가 포트마다 성공/실패 문구를 줄줄이 찍어 실제
>   오류처럼 보이던 문제를 완치했습니다 - 이제 최종 연결된 인스턴스 번호와 포트만 한 줄로 표시됩니다.
> - **원격 정지 시 콘솔창 잔존 결함 완치**: 부팅 시 시스템 부하로 콘솔 PID 캡처가 1회 실패하면 그대로
>   영구 비활성화되던 걸 재시도(3회)로 보강해, 원격 종료 시 콘솔창이 남는 빈도를 줄였습니다.
> - **스크린샷 동기화 캐시 무한 증식 완치**: 삭제된 원본 스크린샷을 계속 들고 있던 캐시 파일을 정리하도록
>   개선했습니다.
> - **정체 복구 중 사망감지 카운터 리셋 누락 완치**: 정체(stuck) 30초 복구 경로의 사망감지 분기가 화면
>   진행 카운터를 리셋하지 않던 쌍둥이 결함을 완치했습니다.
>
> </details>
>
> <details>
> <summary>이전 버전(1.17.1~1.19.1) 변경 이력 보기</summary>
>
> - **(1.19.1) 재부팅 로그 파일명 결함 완치**: 에뮬레이터 콜드 리부트가 연속으로 여러 번 일어나도 로그
>   파일명이 항상 "reboot1" 아니면 "start"로만 찍히던 결함을 완치했습니다. 유령성 4층 착지 스턱도
>   1회차부터 곧바로 전진 스와이프+출구 재탭을 실행하도록 단축했습니다.
>
...
>   체크포인트를 찍어두셔야 합니다. 아래 스크린샷(`readme_img/유령성3층셋팅.png`)을 참고해
>   동일한 위치에 체크포인트를 찍어주세요.
>
>   ![유령성 3층 체크포인트 위치](readme_img/유령성3층셋팅.png)
