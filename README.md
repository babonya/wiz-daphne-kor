# 위저드리 배리언트 다프네 매크로 (Wizardry Variants Daphne Macro) — 위저드리 다프네 한글버전 매크로
...
3. 콘솔 화면(그리고 `remote_control/설정안내.txt`)에 시작/정지/상태확인용 URL이 출력됩니다. (프리셋별로 배치파일을 여러 개 만들어두시면, 그만큼 시작 URL도 자동으로 여러 개 생성됩니다.)
4. 스마트폰의 웹 브라우저에서 그 URL들을 각각 열고 **"홈 화면에 추가"**로 등록하면, 이후로는 아이콘 탭 한 번으로 원격 제어가 가능합니다.
