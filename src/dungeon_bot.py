...
#     (1) "블랙박스 경고"(정체 감지 최초 시점)에 그 순간 화면을 logs/에 자동 저장하는
#     save_stuck_evidence_screenshot() 신설 - 같은 정체 에피소드당 1장만 남긴다. 기존엔 300초
#     하드리밋(main.py의 restart_process)에 도달해야만 증거 스샷이 남아, 정체 타이머가 반복
#     리셋되는 사고에서는 증거가 전혀 안 남는 문제가 있었음.
#     (2) 상자 미니게임 "즉각 돌입 가드"(FIELD_WAIT↔PLAY_MINIGAME 왕복)가 상태 전이 때마다
#     last_state_changed_time을 정상적으로(그러나 결과적으로 부당하게) 리셋시켜, 뮤뮤 자체 동결
#     등으로 미니게임이 실제로는 전혀 안 풀리는데도 300초 하드리밋이 절대 안 걸리던 실전 사고
#     완치(2026-09-18, 같은 왕복이 33초 주기로 554회, 1시간+ 반복). last_state_changed_time과
#     완전히 독립된 전용 카운터(minigame_reentry_count)를 신설해 10회 넘게 왕복하면 강제
#     재시작 - 상자 미니게임 실제 최대 도전 횟수 4회의 배 이상 여유(승/패 확인 없이 난타만
#     하므로). 리셋 조건: 새 "열다" 감지, 상자 완료 후 필드 안착.
#     (그 외 "대화창 저격" 선택지 우선 처리는 main.py 자체 변경 - 상세는 main.py 참고.)
