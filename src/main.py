...
CURRENT_VERSION = "1.21.13" # 📋 [시스템 버전 변수] 업데이트 시 이 버전 수치만 수정하시면 일괄 동기화됩니다.
...
#     선택지 우선 처리 추가, 3건.
#     (1) 던전봇/사령탑 "블랙박스 경고"(정체 감지 최초 시점)에 증거 스크린샷 자동 저장 - 기존엔 300초
#     하드리밋(restart_process) 시점에만 스샷이 남아, 정체 타이머가 반복 리셋되는 사고에서는 증거가
#     전혀 안 남았음.
#     (2) 상자 미니게임 "즉각 돌입 가드"(FIELD_WAIT↔PLAY_MINIGAME 왕복)가 last_state_changed_time을
#     매번 리셋시켜 300초 하드리밋을 무력화하던 결함 완치 - 뮤뮤 자체 동결 상황에서 같은 왕복이 33초
#     주기로 554회, 1시간+ 반복된 실전 사고 확인. 상세는 dungeon_bot.py 참고.
#     (3) "대화창 저격"(부팅 복구 이후 화살표만 보이면 무조건 탭하던 로직)이 "울타리 구멍" 같은 실제
#     선택이 필요한 화면에서 화살표만 계속 눌러 무한 반복하던 사고 완치(실전: 8시간+, 16,661회 반복)
#     - dungeon_bot.py의 _handle_dungeon_interrupt()(이미 검증된 선택지 처리 로직)를 화살표 탭 전에
#     먼저 호출해 알려진 선택지(중립몹/울타리구멍/뼈상인/행상인)부터 처리하도록 변경.
#   1.21.11: 🆕 부팅 복구 분류기(사령탑)의 score_combat 임계값 0.80 → 0.70 하향 - 동결(freeze)
#     상태이상 시각효과가 배속 버튼 인식 점수를 0.752까지만 떨어뜨려(0.80 미달) 명백한 전투 화면을
#     1시간+ 동안 아웃게임으로 오판, dungeon_bot.py로 못 넘어간 실전 사고 완치. 0.80은 git 이력상
#     최초 커밋 때부터 있던 값으로 의도적 튜닝 근거 없음 - dungeon_bot.py가 같은 도장/ROI를 매 전투
#     틱마다 이미 0.70으로 오탐 없이 써온 것에 맞춤. 그 외(던전 진입 대기 15초 상향, 눈보라구간 재개
#     버튼 밝기 판정, 정체 탈출 가드 1회 힐 제한)는 dungeon_bot.py 참고.
#   1.21.10: (이 파일 자체는 변경 없음, 버전 동기화용) 전투 진입 직후 자동전투가 영원히 안 켜져 정체
#     하던 결함 + 대설지대 눈보라구간 재개버튼 무한 반복 결함 완치. 상세는 dungeon_bot.py 참고.
#   1.21.9: (이 파일 자체는 변경 없음, 버전 동기화용) "빈사"(힐로 해결)와 "사망"(힐로 절대 안 풀림)을
#     구분 못 해 정비를 무한 재격발하던 결함(실전 12시간 낭비) 완치 - 사망 확인 시 힐 재시도 억제 +
#     마을 회군 시도 추가. 사망 감지 도장/좌표 재검증까지 완료(정탐 0.999~1.000 vs 오탐 최고 0.585로
#     분리 확인, 상세는 dungeon_bot.py 참고).
#   1.21.8: (이 파일 자체는 변경 없음, 버전 동기화용) 예상 밖 상태 팝업이 화면 과도기로 오인돼 정체
#     되던 상황에서 'X 닫기'를 능동적으로 눌러 치우는 안전장치 추가. 상세는 dungeon_bot.py 참고.
#   1.21.7: (이 파일 자체는 변경 없음, 버전 동기화용) 대설지대 "뼈상인"(뼈 줍는 고블린) 조우 처리
#     신규 추가 - 모험가의 뼈 1순위, 유해를 부르는 기름 2순위 고정 선택. 상세는 dungeon_bot.py 참고.
...
#     사이클이 3시간 넘게 반복됐는데, 사용자가 MuMu 관리자 화면에서 해당 인스턴스가 "실행이 중지됨"
#     상태였고 ADB 연결/화면 캡처는 살아있는데 터치 입력만 죽어 있었음을 직접 확인. 원인은
#     `prev_cursor_dir`(대설지대 전용 커서방향 이동감지 기준값)이 매크로 기동 시 딱 한 번만
#     초기화되고 그 뒤로 다시 갱신되지 않는 구조라, 캐릭터가 실제로는 전혀 못 움직였는데도 "지금
#     방향"이 몇 시간 전의 낡은 기준값과 우연히 다르기만 하면 첫 프레임에서 곧장 "이동함"으로
#     오판됐다. 이 오판이 정체 타이머(last_state_changed_time)까지 매번 리셋해버려, 원래 300초
#     정체 시 자동 재시작해야 할 절대 워치독 자체가 발동 조건(먼저 30초 정체가 쌓여야 함)에 도달하지
#     못했다. "상자 자동 이동" 탭 직전 화면에서 방향을 다시 찍어 매번 새 기준값을 세우도록 완치 -
#     비교 대상이 항상 "방금 전"이 되도록 해서, 터치가 죽어 진짜로 안 움직였다면 정체 타이머가 정상
#     누적돼 기존 300초 워치독이 제대로 작동한다.
#   1.21.4: 🚨 recover_app_startup()이 대설지대 "마을외곽"(Dungeon Select) 화면을 프리셋에 따라
#     반쪽만 인식하던 결함 완전 완치. 사용자가 "이스벨크 마을외곽"에 캐릭터를 세워둔 채 다른 프리셋
#     (유령성 4층 상자파밍, DUNGEON_NAME="북쪽의 유령선")으로 매크로를 켜서 재현 - "언제 어디서 켜도
#     목표 던전으로 찾아간다"는 설계 목표(ROADMAP 항목 8)를 실기로 검증하려다 발견됨. 2026-09-08에
#     이미 한 번 같은 화면 문제를 고쳤었는데, 그 완치가 `t_heavysnow_outskirts`/`t_heavysnow_route`
#     도장을 `DUNGEON_NAME == "대설지대"`일 때만 로드하는 방식이라 - "지금 켠 프리셋이 대설지대일
#     때"만 그 화면을 알아봤다. 오늘 재현된 건 정반대 상황(프리셋은 다른 던전인데 화면이 대설지대
#     마을외곽)이라 여전히 아무 앵커도 안 걸려 빈 (1,1) 탭만 35회 반복하며 정체했다(실전 로그
#     23:05:33~, "로딩/타이틀 정체 감지"). 사용자가 수동으로 "마을외곽"이 아닌 진짜 "마을"로 이동하니
#     `village_common/inn.png`(프리셋 무관 공용 도장)로 즉시 정상 인식된 것으로 원인 교차검증 완료.
#     두 도장을 프리셋 조건 없이 항상 로드하도록 완치 - 다른 던전 앵커(t_dungeon_sel, t_village_anchor
#     등)와 동일하게 "그냥 아는 인게임 화면 중 하나"로 취급하면, 여기서 인게임 진입만 확정해주고 그
#     화면이 지금 프리셋과 안 맞으면 이어지는 메인 루프의 범용 이탈 로직이 세계지도로 내보낸다.
#   1.21.3: 🚨 우물(캠프) 아이콘이 화면 상단에 너무 가까우면 자동이동 버블이 타이틀바에 가려 안
#     보이던 결함 + 그 실패가 곧장 앱 전체 재시작으로 이어지던 과잉 대응 완치(dungeon_bot.py 상세
#     참고). 실전 로그(2026-09-09 19:05): 우물 탭 (1059,373) 후 자동이동 버튼 미검출이 3회 연속돼
#     앱이 강제 재시작됐는데, 사용자가 스크린샷으로 우물이 상단 타이틀바("경로6 -호반(남)-")에 바짝
#     붙어 있음을 확인 - 자동이동 버블이 그 타이틀바에 가려진 것으로 추정. 사용자 지적대로 아이콘
#     탐색에 최소 Y좌표(그 지점보다 200px 아래, 573)를 두어 상단 근접 매칭은 미검출로 간주하고
#     기존 스와이프/재탐색이 더 아래쪽에서 다시 찾도록 완치. 또한 사용자 지적("모든 앵커가 멀쩡한데
#     ADB를 재시작해버리는 것도 문제") 반영 - 자동이동 버튼을 끝내 못 찾아도 곧장 "failed"→
#     RuntimeError→ADB 서버 리셋으로 가지 않고, 기존 "retry" 경로(TRIGGER_EXIT의
#     fieldmap_return_retry_count, 최대 5회)로 먼저 돌려보내 필드맵을 다시 열어 재시도하고, 그마저
#     다 소진했을 때만 앱을 재시작하도록 완화.
...
def recover_app_startup(device):
    ...
    # 🚨 [2026-09-08 실전 확인, 2026-09-09 재발+완전 완치] 대설지대 경유 화면(마을외곽 / 대설지대 경로
    # 목록)은 아래 "인게임 진입 성공" 앵커 목록 어디에도 없어서, 그 화면에서 매크로를 시작하면 아는 화면이
    # 하나도 없다고 판단해 빈 (1,1) 탭만 반복하다 35회 예산을 통째로 날렸다(실전 로그 23:09, 마을외곽에서
    # 기동). 최초 완치는 이 두 도장을 `DUNGEON_NAME == "대설지대"`일 때만 로드했는데, 그러면 "지금 켠
    # 프리셋의 던전은 대설지대가 아닌데, 캐릭터가 대설지대 마을외곽에 서 있는" 반대 상황(사용자 실측
    # 재현: 유령성4층 프리셋을 이스벨크 마을외곽에서 기동)에서 여전히 똑같이 안 잡혔다 - "언제 어디서
    # 켜도 목표 던전으로 찾아간다"는 설계 목표(ROADMAP 항목 8)상 이 두 앵커는 대설지대 프리셋 전용이
    # 아니라 다른 던전 앵커들(t_dungeon_sel, t_village_anchor 등)과 동일하게 "그냥 아는 인게임 화면
    # 중 하나"로 취급해야 한다 - 여기서 True를 반환한 뒤엔 메인 루프의 범용 이탈 로직(마을외곽/던전선택
    # 화면이 지금 프리셋과 안 맞으면 세계지도로 나가는 것)이 알아서 이어받는다. 프리셋과 무관하게 항상
    # 로드하도록 조건을 제거했다. 메인 루프가 이 두 화면을 구분할 때 쓰는 것과 완전히 같은 도장/임계값
    # (0.80)을 그대로 재사용한다.
